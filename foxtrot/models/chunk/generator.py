...
class Generator:
    ...
    Attributes:
        ...
        doors: **list of all grid coordinates of the corridor to room connections, elements are tuples (x,y), empty until connect_all_rooms() is called
        corridors: **list of all the corridor tiles in the grid, elements are tuples (x,y), empty until generate_corridors() is called
        deadends: list of all corridor tiles only connected to one other tile, elements are tuples (x,y), empty until find_deadends() is called
    ...
    def flood_fill(self, x, y, fillWith, tilesToFill=[], grid=None):
        """
    ...
    def find_unconnected_areas(self):
        ...
        in order to use join_unconnected_areas() this needs to be called first and the returned list passed to join_unconnected_areas()
        ...
        for x in range(self.width):
            for y in range(self.height):
                if gridCopy[x][y] == "x":
                    ...
                    areaCount += 1
                    self.flood_fill(x, y, areaCount, None, gridCopy)
        for x in range(self.width):
            for y in range(self.height):
                if gridCopy[x][y]:
                    i = gridCopy[x][y]
    ...
    def find_deadends(self):
        ...
        populates self.deadends and is used by prune_deadends()
    ...
    def place_room(self, startX, startY, roomWidth, roomHeight, ignoreOverlap=False):
        """
        ...
        if self.quad_fits(startX, startY, roomWidth, roomHeight, 0) or ignoreOverlap:
            ...
            self.rooms.append(Room(startX, startY, roomWidth, roomHeight))
            return True
    ...
    def place_walls(self):
        ...
        As some functions (like flood_fill() and anything that uses it) dont distinguish between tile types it is best called later/last
    ...
    def join_unconnected_areas(self, unconnectedAreas):
        """
        ...
        Args:
            unconnectedAreas: the list returned by find_unconnected_areas() - ie [[(x1,y1), (x2,y2), (x3,y3)], [(xi1,yi1), (xi2,yi2), (xi3,yi3)]]
